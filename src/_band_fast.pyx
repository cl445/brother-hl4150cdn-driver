...
through the inverse LUT (or the no-colour-matching split), optional tone
...
cdef int[4] _READ_GROUP = [12, 1, 1, 12]
...
def render_band(
    ...
    Args:
        ...
        lut: flat (256*256*256*4) KCMY inverse LUT, or empty for the
            no-colour-matching split (K = 255, C/M/Y = R/G/B).
